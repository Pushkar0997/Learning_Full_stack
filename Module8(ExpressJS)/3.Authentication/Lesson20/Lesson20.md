...
When Checkout button is clicked, it should delete all items in the cart for that user.

Challenge:
1. Delete all cart items for a user.

Inside `cartController.js`, we add the `checkout` function:
...
```
Here, we simply run a DELETE query to remove all items from the `cart_items` table for the logged-in user. After the deletion, we send a 204 No Content status code to indicate that the operation was successful and there is no content to return.
...
```
This code now includes the `deleteAll` function which will be called when the user clicks the Checkout button to clear their cart.
