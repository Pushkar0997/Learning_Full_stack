...
/*
Reasons why a model can stope generating further tokens:
1. It reaches the maximum token limit set for the response.
2. It encounters a predefined stop sequence in the input prompt.
3. It completes the task or answers the question posed in the prompt.
4. It detects a safety filter or content policy violation.
5. The API request is cancelled or interrupted.
and more...

The Stop Sequence:
- A stop sequence is a specific set of characters or tokens that signal the model to stop generating further text.
- It is used to control the output of the model and prevent it from generating unwanted or excessive text.
- Stop sequences can be defined by the user when making an API request to the language model.
- An array with upto 4 sequences can be provided.
- The models stops generating further tokens when it encounters any of the defined stop sequences in the generated text.
- The outputted text will not include the stop sequence itself.
